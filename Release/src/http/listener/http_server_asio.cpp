...
using utility::details::str_iless;
...
private:
    void on_accept(std::unique_ptr<asio::ip::tcp::socket> socket, const asio::error_code& ec);
};
        ...
        bool is_https, const std::function<void(asio::ssl::context&)>& ssl_context_callback)
    {
        ...
        if (is_https)
        {
            ...
            m_ssl_stream->async_handshake(
                ...
                [this](const asio::error_code&) { (will_deref_and_erase_t) this->start_request_response(); });
...
private:
    ...
    will_deref_and_erase_t start_request_response();
    will_deref_and_erase_t handle_http_line(const asio::error_code& ec);
    will_deref_and_erase_t handle_headers();
    will_deref_t handle_body(const asio::error_code& ec);
    will_deref_t handle_chunked_header(const asio::error_code& ec);
    will_deref_t handle_chunked_body(const asio::error_code& ec, int toWrite);
    ...
    will_deref_and_erase_t handle_headers_written(const http_response& response, const asio::error_code& ec);
    ...
    will_deref_and_erase_t handle_write_chunked_response(const http_response& response,
                                                         const asio::error_code& ec);
    will_deref_and_erase_t handle_response_written(const http_response& response, const asio::error_code& ec);
...
will_deref_and_erase_t asio_server_connection::start_request_response()
{
    ...
    if (m_ssl_stream)
    {
        asio::async_read_until(
            *m_ssl_stream, m_request_buf, CRLFCRLF, [this](const asio::error_code& ec, std::size_t) {
...
void hostport_listener::on_accept(std::unique_ptr<ip::tcp::socket> socket, const asio::error_code& ec)
{
    // Listener closed
    ...
    if (m_acceptor)
    {
        ...
        m_acceptor->async_accept(*newSocket, [this, newSocket](const asio::error_code& ec) {
            std::unique_ptr<ip::tcp::socket> usocket(newSocket);
            this->on_accept(std::move(usocket), ec);
        });
        usocket.release();
...
will_deref_and_erase_t asio_server_connection::handle_http_line(const asio::error_code& ec)
{
    ...
    else
    {
        ...
        if (str_iequal(http_verb, methods::GET))
            http_verb = methods::GET;
        else if (str_iequal(http_verb, methods::POST))
            http_verb = methods::POST;
        else if (str_iequal(http_verb, methods::PUT))
            http_verb = methods::PUT;
        else if (str_iequal(http_verb, methods::DEL))
            http_verb = methods::DEL;
        else if (str_iequal(http_verb, methods::HEAD))
            http_verb = methods::HEAD;
        else if (str_iequal(http_verb, methods::TRCE))
            http_verb = methods::TRCE;
        else if (str_iequal(http_verb, methods::CONNECT))
            http_verb = methods::CONNECT;
        else if (str_iequal(http_verb, methods::OPTIONS))
...
will_deref_and_erase_t asio_server_connection::handle_headers()
{
    ...
    while (std::getline(request_stream, header) && header != "\r")
    {
        ...
        if (colon != std::string::npos && colon != 0)
        {
            ...
            if (str_iequal(name, header_names::content_length))
            {
    ...
    {
        m_close = str_iequal(name, U("close"));
    }

    if (currentRequest.headers().match(header_names::transfer_encoding, name))
    {
        m_chunked = name.find_first_of (U("chunked")) != utility::string_t::npos;
    }
    ...
    else // need to read the sent data
    {
        ...
        async_read_until_buffersize(
            ...
            [this](const asio::error_code& ec, size_t) { (will_deref_t) this->handle_body(ec); });
    }

    return dispatch_request_to_listener();
}

will_deref_t asio_server_connection::handle_chunked_header(const asio::error_code& ec)
{
    ...
    else
    {
        ...
        {
            async_read_until_buffersize(len + 2, [this, len](const asio::error_code& ec, size_t) {
...
will_deref_t asio_server_connection::handle_chunked_body(const asio::error_code& ec, int toWrite)
{
    auto requestImpl = get_request()._get_impl();
    if (ec)
    {
...
will_deref_t asio_server_connection::handle_body(const asio::error_code& ec)
{
    auto requestImpl = get_request()._get_impl();
    // read body
    if (ec)
    ...
    else if (m_read < m_read_size) // there is more to read
    {
        auto writebuf = requestImpl->outstream().streambuf();
        writebuf
            .putn_nocopy(asio::buffer_cast<const uint8_t*>(m_request_buf.data()),
...
will_deref_and_erase_t asio_server_connection::async_write(WriteFunc response_func_ptr, const http_response& response)
{
    if (m_ssl_stream)
    {
        asio::async_write(*m_ssl_stream, m_response_buf, [=](const asio::error_code& ec, std::size_t) {
            (this->*response_func_ptr)(response, ec);
        });
    }
    else
    {
        asio::async_write(*m_socket, m_response_buf, [=](const asio::error_code& ec, std::size_t) {
            (this->*response_func_ptr)(response, ec);
        });
    }
    return will_deref_and_erase_t {};
...
will_deref_t asio_server_connection::async_handle_chunked_header()
{
    ...
    {
        asio::async_read_until(
            *m_ssl_stream, m_request_buf, CRLF, [this](const asio::error_code& ec, size_t) {
    ...
    {
        asio::async_read_until(
            *m_socket, m_request_buf, CRLF, [this](const asio::error_code& ec, size_t) {
                (will_deref_t) this->handle_chunked_header(ec);
            });
    }
    return will_deref_t {};
...
void asio_server_connection::serialize_headers(http_response response)
{
    ...
    for (const auto& header : response.headers())
    {
        ...
        if (str_iequal(header.first, U("connection")))
        {
            if (str_iequal(header.second, U("close")))
            {
                m_close = true;
            }
        }
